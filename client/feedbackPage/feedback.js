...
async function loadLeaderboard() {
  try {
    const response = await fetch("http://localhost:3000/user_response");
    ...
    const feedbackSection = document.querySelector("article");
        ...
        let userFeedback = `<p>Great job, <b>${loggedInUser || "Guest"}</b>! You didn't make any mistakes.</p>`;
      ...
      if (latestUserResponse) {
        const incorrectCategories = latestUserResponse.incorrect_categories?.length
        ...
        : "None";
          ...
          if (latestUserResponse.incorrect_categories?.length) {
            const categoryFeedback = generateFeedback(latestUserResponse.incorrect_categories);
...
function generateFeedback(incorrectCategories) {
  let feedbackMessages = {
    "Weather": "It looks like you're struggling with Weather concepts. Review climate patterns and meteorological effects.",
    "Climate Change": "Brush up on Climate Change topics! Consider studying greenhouse gases and global warming effects.",
    "Natural Disasters": "You may need to revisit Natural Disasters. Focus on causes and responses to earthquakes, tsunamis, and hurricanes.",
    "Geography": "Improve your Geography knowledge! Try learning more about continents, maps, and landforms.",
    "Science": "Science concepts can be tricky! Consider reviewing fundamental scientific principles.",
  };

  let feedback = incorrectCategories.map(category => feedbackMessages[category] || `Keep practicing on ${category}!`);
